...
aios::AiosCredentials Settings::GetAiosCredentials() {
    aios::AiosCredentials credentials;
    credentials.device_user_id = GetString("device_user_id");
    credentials.access_key = GetString("access_key");
    credentials.access_secret = GetString("access_secret");
    credentials.token = GetString("token");
    credentials.ws_url = GetString("ws_url");
    return credentials;
}

void Settings::SetAiosCredentials(const aios::AiosCredentials& credentials) {
    SetString("device_user_id", credentials.device_user_id);
    SetString("access_key", credentials.access_key);
    SetString("access_secret", credentials.access_secret);
    SetString("token", credentials.token);
    SetString("ws_url", credentials.ws_url);
}

void Settings::ClearAiosCredentials() {
    EraseKey("device_user_id");
    EraseKey("access_key");
    EraseKey("access_secret");
    EraseKey("token");
    EraseKey("ws_url");
}
