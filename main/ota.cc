...
esp_err_t Ota::CheckVersion() {
    ...
    if (strlen(CONFIG_AIOS_API_BASE_URL) > 0) {
        AiosAuthClient aios_auth;
        AiosWebsocketConfig ws_config;
        std::string auth_error;
        if (!aios_auth.FetchWebsocketConfig(ws_config, auth_error)) {
            ESP_LOGE(TAG, "AIOS auth failed: %s", auth_error.c_str());
            return ESP_FAIL;
        }

        Settings ws_settings("websocket", true);
        ws_settings.SetString("url", ws_config.url);
        ws_settings.SetString("token", ws_config.token);
        ws_settings.SetString("x-device-id", ws_config.device_id);
        ws_settings.SetString("x-user-id", ws_config.user_id);
        ws_settings.SetString("x-bot-id", ws_config.bot_id);
        ws_settings.SetString("x-aios-version", ws_config.version);
        ws_settings.SetBool("aios_mode", true);
        ...
        ESP_LOGI(TAG, "AIOS websocket config ready, ws=%s, device_id=%s, bot_id=%s",
                 ws_config.url.c_str(), ws_config.device_id.c_str(), ws_config.bot_id.c_str());
        return ESP_OK;
    }
#endif
    ...
    if (cJSON_IsObject(websocket)) {
        ...
        settings.SetBool("aios_mode", true);
        settings.EraseKey("token");
        settings.EraseKey("x-bot-id");
        settings.EraseKey("x-device-id");
        settings.EraseKey("x-user-id");
        settings.EraseKey("x-aios-version");
        cJSON *item = NULL;
