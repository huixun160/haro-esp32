...
#ifdef CONFIG_USE_VOLUME_WAKE_WORD
#include "wake_words/volume_wake_word.h"
#endif
    ...
    if (codec_->input_sample_rate() != sample_rate) {
        ...
        if (!codec_->InputData(data)) {
            ...
            if (input_resampler_ != nullptr) {
                ...
                if (esp_ae_rate_cvt_get_max_out_sample_num(input_resampler_, in_sample_num, &output_samples) != ESP_AE_ERR_OK ||
                    output_samples == 0 || output_samples > 48000) {
                    ESP_LOGE(TAG, "Invalid input resampler target size: in=%u out=%u", in_sample_num, output_samples);
                    return false;
                }
                auto resampled = std::vector<int16_t>(output_samples * codec_->input_channels());
                uint32_t actual_output = output_samples;
                if (esp_ae_rate_cvt_process(input_resampler_, (esp_ae_sample_t)data.data(), in_sample_num,
                                            (esp_ae_sample_t)resampled.data(), &actual_output) != ESP_AE_ERR_OK) {
                    ESP_LOGE(TAG, "Input resampler process failed");
                    return false;
                }
                if (actual_output > output_samples) {
                    ESP_LOGW(TAG, "Input resampler overflow guard: actual=%u target=%u", actual_output, output_samples);
                    actual_output = output_samples;
                }
...
void AudioService::OpusCodecTask() {
    while (true) {
        ...
        if (!audio_decode_queue_.empty() && audio_playback_queue_.size() < MAX_PLAYBACK_TASKS_IN_QUEUE) {
            ...
            if (packet->format == AudioStreamPacket::PayloadFormat::PCM_S16LE) {
                if ((packet->payload.size() % sizeof(int16_t)) != 0) {
                    ESP_LOGW(TAG, "Drop odd-sized PCM packet: %u bytes", (unsigned)packet->payload.size());
                    lock.lock();
                    continue;
                }

                task->pcm.resize(packet->payload.size() / sizeof(int16_t));
                std::memcpy(task->pcm.data(), packet->payload.data(), packet->payload.size());
                if (decoder_sample_rate_ != codec_->output_sample_rate() && output_resampler_ != nullptr) {
                    uint32_t target_size = 0;
                    if (esp_ae_rate_cvt_get_max_out_sample_num(output_resampler_, task->pcm.size(), &target_size) != ESP_AE_ERR_OK ||
                        target_size == 0 || target_size > 48000) {
                        ESP_LOGE(TAG, "Invalid output resampler target size (pcm): in=%u out=%u",
                                 (unsigned)task->pcm.size(), (unsigned)target_size);
                        lock.lock();
                        continue;
                    }
                    // NOTE: Some firmware/library combinations may report an underestimated
                    // max output size. Reserve very large headroom to avoid memory overwrite.
                    uint32_t capacity = target_size * 8;
                    uint32_t alt_capacity = static_cast<uint32_t>(task->pcm.size() * 12 + 1024);
                    if (capacity < alt_capacity) {
                    ...
                    std::vector<int16_t> resampled(capacity);
                    uint32_t actual_output = capacity;
                    if (esp_ae_rate_cvt_process(output_resampler_, (esp_ae_sample_t)task->pcm.data(), task->pcm.size(),
                                                (esp_ae_sample_t)resampled.data(), &actual_output) != ESP_AE_ERR_OK) {
                        ESP_LOGE(TAG, "Output resampler process failed (pcm)");
                        lock.lock();
                        continue;
                    }
                    if (actual_output > capacity) {
                        ESP_LOGE(TAG, "Output resampler overflow guard (pcm): actual=%u capacity=%u, dropping tail",
                                 (unsigned)actual_output, (unsigned)capacity);
            ...
            if (opus_decoder_ != nullptr) {
                ...
                if (ret == ESP_AUDIO_ERR_OK) {
                    ...
                    if (decoder_sample_rate_ != codec_->output_sample_rate() && output_resampler_ != nullptr) {
                        ...
                        if (esp_ae_rate_cvt_get_max_out_sample_num(output_resampler_, task->pcm.size(), &target_size) != ESP_AE_ERR_OK ||
                            target_size == 0 || target_size > 48000) {
                            ESP_LOGE(TAG, "Invalid output resampler target size (opus): in=%u out=%u",
                                     (unsigned)task->pcm.size(), (unsigned)target_size);
                            lock.lock();
                            continue;
                        }
                        ...
                        if (esp_ae_rate_cvt_process(output_resampler_, (esp_ae_sample_t)task->pcm.data(), task->pcm.size(),
                                                    (esp_ae_sample_t)resampled.data(), &actual_output) != ESP_AE_ERR_OK) {
                            ESP_LOGE(TAG, "Output resampler process failed (opus)");
                            lock.lock();
                            continue;
                        }
                        if (actual_output > capacity) {
                            ESP_LOGE(TAG, "Output resampler overflow guard (opus): actual=%u capacity=%u, dropping tail",
                                     (unsigned)actual_output, (unsigned)capacity);
                            actual_output = capacity;
                        }
                        resampled.resize(actual_output);
...
#ifdef CONFIG_USE_VOLUME_WAKE_WORD
    wake_word_ = std::make_unique<VolumeWakeWord>();
#else
#if CONFIG_IDF_TARGET_ESP32S3 || CONFIG_IDF_TARGET_ESP32P4
