...
void Otto::CuriousLookAround() {
    ...
    int center_head = 90; // 默认头部位置 (pitch, 上下限位 70/110)
    int center_base = 90; // 默认底座位置 (yaw)
    ...
    LookTarget sequence[] = {
        // === 开场：缓慢环顾 ~15s ===
        {0, 8, 1.2, 0.8}, {30, 5, 1.2, 1.0}, {55, 8, 1.0, 1.2}, {0, 0, 1.5, 0.8},
        {-35, 5, 1.0, 0.8}, {-55, 3, 1.0, 1.2}, {0, 0, 1.5, 0.8},
        
        // === 发呆凝视 ~14s ===
        {8, 12, 1.2, 2.0}, {12, 8, 0.8, 2.0}, {6, 10, 0.6, 1.5}, {3, 6, 0.8, 1.5}, {0, 0, 1.0, 0.8},
        
        // === 低头沉思 ~15s ===
        {15, -14, 1.0, 1.2}, {22, -20, 0.8, 1.5}, {-18, -16, 1.0, 1.2}, {-22, -20, 0.8, 1.5}, {0, 0, 1.2, 1.0},
        
        // === 突然警觉！大幅扫视 ~9s ===
        {-80, 5, 0.6, 0.5}, {80, 0, 0.7, 0.5}, {80, -15, 0.35, 0.3}, {80, 15, 0.35, 0.3},
        {-40, 0, 0.5, 0.3}, {40, 0, 0.5, 0.3}, {0, 10, 0.8, 1.0}, {0, 0, 1.0, 1.0},
        
        // === 打量右侧 ~12s ===
        {45, 0, 1.2, 0.8}, {55, 14, 0.8, 1.0}, {50, -8, 0.8, 1.0}, {50, 5, 0.7, 1.2}, {0, 0, 1.2, 0.8},
        
        // === 点头，若有所思 ~8s ===
        {0, -10, 0.8, 0.5}, {0, 8, 0.8, 0.5}, {0, -10, 0.8, 0.5}, {0, 5, 0.8, 0.6}, {0, 0, 0.7, 0.8},
        
        // === 仰望 ~13s ===
        {12, 20, 1.0, 1.5}, {-12, 22, 1.0, 1.8}, {0, 18, 0.8, 1.5}, {5, 15, 0.8, 1.0}, {0, 0, 1.2, 0.8},
        
        // === 突然大幅转向！点头交流 ~10s ===
        {-75, 10, 0.7, 1.0}, {-75, -18, 0.4, 0.4}, {-75, 15, 0.4, 0.4}, {-75, 0, 0.3, 0.8},
        {-85, 18, 0.5, 1.0}, {-75, 0, 0.5, 0.8}, {0, 8, 0.8, 0.8}, {0, 0, 1.2, 0.8},
        
        // === 目光游荡 ~12s ===
        {28, 14, 1.0, 0.8}, {-28, 18, 1.2, 0.8}, {-40, 5, 0.8, 1.0}, {22, -10, 1.0, 0.8}, {5, 12, 0.8, 0.8}, {0, 0, 1.0, 0.8},
        
        // === 放空 ~10s ===
        {6, 4, 1.5, 2.0}, {-5, 6, 1.2, 2.0}, {0, 2, 1.0, 1.5}, {0, 0, 0.8, 0.5},
        
        // === 全景收尾 ~14s ===
        {-65, 8, 1.8, 1.0}, {-65, -10, 0.7, 0.8}, {65, 0, 2.2, 1.0}, {65, 10, 0.7, 0.8}, {0, 0, 1.8, 0.6}
    };

    int num_steps = sizeof(sequence) / sizeof(sequence[0]);

    // 完全一比一复刻 Python 的 smooth_move (Cosine Easing, 50fps)
    auto smooth_move = [&](float to_yaw, float to_pitch, float duration) {
        float from_yaw = servo_[RIGHT_HAND].GetPosition() - center_base;
        float from_pitch = center_head - servo_[LEFT_HAND].GetPosition(); // 还原当前的 pitch

        int steps = std::max(1, (int)round(duration / 0.02f)); // 50 fps
        for (int i = 1; i <= steps; i++) {
            float t = (float)i / steps;
            float eased_t = (1.0f - cos(t * M_PI)) / 2.0f; // ease-in-out
            
            float current_yaw = from_yaw + eased_t * (to_yaw - from_yaw);
            float current_pitch = from_pitch + eased_t * (to_pitch - from_pitch);
            
            servo_[RIGHT_HAND].SetPosition(center_base + (int)round(current_yaw));
            servo_[LEFT_HAND].SetPosition(center_head - (int)round(current_pitch));
            
            vTaskDelay(pdMS_TO_TICKS(20)); // 严格按照 Python 的 time.sleep(0.02)
        }
        
        // 确保最终位置精确
        servo_[RIGHT_HAND].SetPosition(center_base + (int)round(to_yaw));
        servo_[LEFT_HAND].SetPosition(center_head - (int)round(to_pitch));
    };

    for (int i = 0; i < num_steps; i++) {
        smooth_move(sequence[i].yaw, sequence[i].pitch, sequence[i].move_time);
        if (sequence[i].pause_time > 0.0f) {
            vTaskDelay(pdMS_TO_TICKS((int)(sequence[i].pause_time * 1000)));
        }
    }
    ...
    // 平滑地回到中间位置
    int home_target[SERVO_COUNT];
    for (int j = 0; j < SERVO_COUNT; j++) {
        home_target[j] = servo_[j].GetPosition();
    }
    home_target[LEFT_HAND] = center_head;
    home_target[RIGHT_HAND] = center_base;
    MoveServos(800, home_target);
}
